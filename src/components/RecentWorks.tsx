...
interface RecentWorksProps extends React.HTMLAttributes<HTMLDivElement> {
  className?: string;
}

function RecentWorks({ className, ...props }: RecentWorksProps) {
    return (
        <div
            className={`col-span-4 row-span-4 bg-blue-100/70 rounded-2xl p-4 hover:shadow-lg transition-all animate-pop-up ${
                className || ''
            }`}
            {...props}
        >
